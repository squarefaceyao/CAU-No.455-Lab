...
def writeTrainFile(Matrix):
    Matrix = np.triu(Matrix)
    index = np.argwhere(Matrix != 0)
    Train_File = osp.join(current_dir,'..', 'Data', 'ara-protein', 'split', 'Train.txt')

    with open(Train_File, 'w') as file:
        np.savetxt(file, index, fmt='%d')

def writeTestFile(Matrix):
    Matrix = np.triu(Matrix)
    index = np.argwhere(Matrix != 0)
    Train_File = osp.join(current_dir,'..', 'Data', 'ara-protein', 'split', 'Test.txt')
    with open(Train_File, 'w') as file:
        np.savetxt(file, index, fmt='%d')
...
def Divide(NetFile, MatrixAdjacency_Net, MaxNodeNum):
    print("Divide......")
    ...
    Train_test_File = osp.join(current_dir, '..', 'Data', 'ara-protein', 'split')

    folder = osp.exists(Train_test_File)
    if not folder:
        os.makedirs(Train_test_File)
    writeTrainFile(MatrixAdjacency_Train)
    writeTestFile(MatrixAdjacency_Test)
